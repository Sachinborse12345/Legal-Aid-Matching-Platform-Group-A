...
export default function CitizenAnalytics({ profile }) {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/citizens/${citizenId}/analytics`,
      );
      ...
      toast.error(
        `Failed to load analytics data: ${error.response?.data?.error || error.message || "Unknown error"}`,
      );
      setData(null);
    } finally {
      setLoading(false);
  ...
  // Process Chart Data
  const monthlyCasesData = Object.keys(data.monthlyCases || {}).map(
    (month) => ({
      name: month,
  ...
  const monthlyAppointmentsData = Object.keys(
  ...
  const casesByStatusData = Object.keys(data.casesByStatus || {}).map(
    (status) => ({
      name: status,
  ...
  const appointmentsByStatusData = Object.keys(
    ...
    <div className="space-y-6 animate-fade-in p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-[#1a1a1a] rounded-xl p-6 border border-gray-200 dark:border-[#333] shadow-lg flex flex-col justify-center items-center">
          ...
          <div className="text-center">
            <div className="text-4xl font-bold text-[#D4AF37] mb-1">
              {data.confirmationRate?.toFixed(1) || 0}%
            </div>
          ...
          <div className="mt-6 w-full bg-gray-100 dark:bg-[#333] rounded-full h-2 overflow-hidden">
            <div
              ...
              style={{ width: `${data.confirmationRate || 0}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#1a1a1a] rounded-xl p-6 border border-gray-200 dark:border-[#333] shadow-lg flex flex-col justify-center items-center">
          ...
          <div className="text-4xl font-bold text-[#E9C46A] mb-1">
                ...
                <PieChart>
                  ...
                  >
                    {casesByStatusData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={CASES_COLORS[index % CASES_COLORS.length]}
                      />
                    ))}
                  </Pie>
                  <Tooltip
                  ...
                  >
                    {appointmentsByStatusData.map((entry, index) => (
                      <Cell
                        ...
                        fill={
                          APPOINTMENTS_COLORS[
                            index % APPOINTMENTS_COLORS.length
                          ]
                        }
                      />
                    ))}
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {completedCases.length} case(s) successfully resolved
              </p>
            </div>
          </div>
      ...
      {completedCases.length === 0 && (
        <div className="mt-8 bg-gray-50 dark:bg-[#1a1a1a] rounded-xl p-8 border border-gray-200 dark:border-[#333] text-center">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500">
            Your completed cases will appear here once they are resolved.
          </p>
        </div>
      )}
    </div>
