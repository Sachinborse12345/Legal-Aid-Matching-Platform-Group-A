...
export default function Sidebar({
  ...
  const profile =
    reduxProfile &&
    (reduxProfile.email || reduxProfile.fullName || reduxProfile.ngoName)
      ? {
          ...reduxProfile,
          displayName:
            reduxProfile.fullName ||
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-4 mb-2">
          <div className="w-12 h-12 border border-[#D4AF37] rounded-full flex items-center justify-center bg-[#D4AF37]/5">
            <img
              src={appLogo}
              alt="Logo"
              className="w-6 h-6 object-contain brightness-0 dark:invert transition-all"
            />
          </div>
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white font-serif tracking-tight uppercase transition-colors">
              AdvoCare
            </h1>
            <span className="text-[10px] text-[#D4AF37] tracking-[0.2em] font-bold uppercase">
              Justice Platform
            </span>
          </div>
        </div>
      ...
      <div className="p-6">
        <div className="bg-gray-50 dark:bg-[#1a1a1a] p-4 rounded-xl border border-gray-200 dark:border-[#333] flex items-center gap-4 shadow-inner ring-1 ring-black/5 dark:ring-white/5 transition-colors">
          ...
          <div className="min-w-0">
            <div className="font-bold truncate text-gray-900 dark:text-white text-sm transition-colors">
              {profile.displayName}
            </div>
            <div className="text-[10px] text-[#D4AF37] font-bold uppercase tracking-wider mt-0.5">
      ...
      <nav className="flex-1 px-4 mt-2 space-y-2 overflow-y-auto custom-scrollbar">
        <div className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] px-4 mb-4 mt-6">
          Main Navigation
        </div>
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left px-5 py-3.5 rounded-lg transition-all cursor-pointer font-bold text-sm tracking-wide flex items-center gap-3 group relative ${
                isActive
      ...
      <div className="p-6 pt-2 border-t border-gray-200 dark:border-[#222]">
        ...
        <div className="mt-4 text-center">
          <p className="text-[9px] text-gray-400 dark:text-gray-600 font-bold uppercase tracking-widest transition-colors">
            © 2026 ADVOCARE LEGAL
          </p>
        </div>
      </div>
