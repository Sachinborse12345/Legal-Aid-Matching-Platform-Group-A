...
const getIcon = (key) => {
  switch (key) {
    ...
    case "find":
      return <FiBriefcase size={20} />; // Or search icon
    case "addcase":
      return <FiBriefcase size={20} />; // Or plus icon
    default:
      return <FiGrid size={20} />;
  }
};
...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-72 bg-[#0a0a0a] border-r border-[#222] z-50 transition-transform duration-300 ease-out ${
          ...
          <nav className="flex-1 px-4 space-y-2 overflow-y-auto custom-scrollbar">
            ...
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  <span
                    className={`${isActive ? "text-black" : "text-[#D4AF37] group-hover:text-white"} transition-colors`}
                  >
                    {getIcon(item.key)}
                  </span>
                  <span
                    className={`font-medium text-sm tracking-wide ${isActive ? "font-bold" : ""}`}
                  >
                    {item.label}
                  </span>
                  {isActive && (
