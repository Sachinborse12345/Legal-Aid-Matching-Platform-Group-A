...
export default function LawyerAnalytics({ profile }) {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/lawyers/${lawyerId}/analytics`,
      );
      ...
      toast.error(
        `Failed to load analytics data: ${error.response?.data?.message || error.message || "Unknown error"}`,
      );
      setData(null);
    } finally {
      setLoading(false);
  ...
  const fetchComparisonData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/lawyers/${comparisonLawyer.id}/analytics`,
      );
      ...
      <div className="flex items-center justify-center h-full">
        <div className="text-center p-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl max-w-md">
          ...
          <p className="text-yellow-700 dark:text-yellow-300 mb-4">
            Unable to load profile information. Please refresh the page or log
            in again.
          </p>
          <button
            onClick={() => {
      ...
      <div className="flex items-center justify-center h-full">
        <div className="text-center p-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl">
          ...
          <p className="text-red-700 dark:text-red-300 mb-4">
            Unable to fetch analytics data. Please check your connection and try
            again.
          </p>
          <button
  ...
  const comparisonMonthlyData = comparisonData
    ? Object.keys(data.monthlyTotal).map((month) => {
  ...
  const radarData = comparisonData
    ? [
        {
          subject: "Total Apps",
          You: Math.min(data.allTimeAppointments / 100, 100),
          Competitor: Math.min(comparisonData.allTimeAppointments / 100, 100),
          fullMark: 100,
        },
        {
          subject: "Conf Rate",
          You: data.confirmationRate,
          Competitor: comparisonData.confirmationRate,
          fullMark: 100,
        },
        {
          subject: "Confirmed",
          You: Math.min(
            (data.allTimeConfirmedAppointments / data.allTimeAppointments) *
              100 || 0,
            100,
          ),
          Competitor: Math.min(
            (comparisonData.allTimeConfirmedAppointments /
              comparisonData.allTimeAppointments) *
              100 || 0,
            100,
          ),
          fullMark: 100,
        ...
        {
          subject: "This Month",
          You: data.totalAppointments,
          Competitor: comparisonData.totalAppointments,
          fullMark: Math.max(
            data.totalAppointments,
            comparisonData.totalAppointments,
            50,
          ),
        },
      ]
        ...
        <div className="flex items-center gap-4">
          ...
          {comparisonMode && (
            <select
              ...
              onChange={(e) => {
                const selected = lawyersList.find(
                ...
                <div className="flex items-center gap-6">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-500">
                      {comparisonLawyer.specialization} •{" "}
                      {comparisonLawyer.city}, {comparisonLawyer.state}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-[#6B9080] rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="text-4xl font-bold mb-2">
                {data.allTimeAppointments}
              </div>
              <div className="text-sm mb-3 opacity-75">
                vs {comparisonData.allTimeAppointments}
              </div>
              <div className="h-2 bg-white/20 rounded-full overflow-hidden">
                <div
                  className="h-full bg-white rounded-full"
                  style={{
                    width: `${Math.min((data.allTimeAppointments / Math.max(data.allTimeAppointments, comparisonData.allTimeAppointments, 1)) * 100, 100)}%`,
                  }}
                ></div>
              </div>
              <div className="text-xs mt-2 opacity-75">
                {data.allTimeAppointments >= comparisonData.allTimeAppointments
                  ? "✓ Leading"
                  : "→ Behind"}
              </div>
            </div>
          ...
          <div className="bg-[#4A7C59] rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="text-4xl font-bold mb-2">
                {data.confirmationRate.toFixed(1)}%
              </div>
              <div className="text-sm mb-3 opacity-75">
                vs {comparisonData.confirmationRate.toFixed(1)}%
              </div>
              <div className="h-2 bg-white/20 rounded-full overflow-hidden">
                <div
                  className="h-full bg-white rounded-full"
              ...
              <div className="text-xs mt-2 opacity-75">
                {data.confirmationRate >= comparisonData.confirmationRate
                  ? "✓ Leading"
                  : "→ Behind"}
              </div>
            </div>
          ...
          <div className="bg-[#E9C46A] rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="text-sm mb-3 opacity-75">
                vs {comparisonData.totalAppointments}
              </div>
              <div className="h-2 bg-white/20 rounded-full overflow-hidden">
                <div
                  className="h-full bg-white rounded-full"
                  style={{
                    width: `${Math.min((data.totalAppointments / Math.max(data.totalAppointments, comparisonData.totalAppointments, 1)) * 100, 100)}%`,
                  }}
                ></div>
              </div>
              <div className="text-xs mt-2 opacity-75">
                {data.totalAppointments >= comparisonData.totalAppointments
                  ? "✓ Leading"
                  : "→ Behind"}
              </div>
            </div>
          ...
          <div className="bg-[#D4AF37] rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="text-4xl font-bold mb-2">
                {data.allTimeConfirmedAppointments}
              </div>
              <div className="text-sm mb-3 opacity-75">
                vs {comparisonData.allTimeConfirmedAppointments}
              </div>
              <div className="h-2 bg-white/20 rounded-full overflow-hidden">
                <div
                  className="h-full bg-white rounded-full"
                  style={{
                    width: `${Math.min((data.allTimeConfirmedAppointments / Math.max(data.allTimeConfirmedAppointments, comparisonData.allTimeConfirmedAppointments, 1)) * 100, 100)}%`,
                  }}
                ></div>
              </div>
              <div className="text-xs mt-2 opacity-75">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-[#1a1a1a] rounded-xl p-6 border border-gray-200 dark:border-[#333] shadow-lg flex flex-col h-96">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
              Monthly Trends Comparison (Last 6 Months)
            </h3>
          ...
          <div className="lg:col-span-2 bg-white dark:bg-[#1a1a1a] rounded-xl p-6 border border-gray-200 dark:border-[#333] shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <h4 className="font-semibold text-[#D4AF37] text-sm uppercase tracking-wide">
                  Your Metrics
                </h4>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-[#222] rounded-lg">
                    ...
                    <span className="font-bold text-gray-900 dark:text-white">
                      {data.allTimePendingAppointments}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-[#222] rounded-lg">
                    <span className="text-sm text-gray-600 dark:text-gray-400">
              ...
              <div className="space-y-4">
                <h4 className="font-semibold text-[#6B9080] text-sm uppercase tracking-wide">
                  Competitor Metrics
                </h4>
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-[#222] rounded-lg">
                  ...
                  <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-[#222] rounded-lg">
                    ...
                    <span className="font-bold text-gray-900 dark:text-white">
                      {comparisonData.allTimePendingAppointments}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-[#222] rounded-lg">
                    <span className="text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <span className="font-bold text-gray-900 dark:text-white">
                      {comparisonData.confirmationRate.toFixed(1)}%
                    </span>
                  </div>
                </div>
              </div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-[#333]">
                {data.upcomingAppointments.length > 0 ? (
                  data.upcomingAppointments.map((appt) => (
                    <tr
                      key={appt.id}
                      className="hover:bg-gray-50 dark:hover:bg-[#222]/50 transition-colors"
                    >
            ...
            {data.recentInteractions && data.recentInteractions.length > 0 ? (
              <div className="divide-y divide-gray-100 dark:divide-[#333]">
                {data.recentInteractions.map((interaction) => (
                  <div
                    key={interaction.id}
                    className="p-4 hover:bg-gray-50 dark:hover:bg-[#222] transition-colors flex items-start gap-3 cursor-pointer"
                    onClick={handleViewMessages}
                  >
              ...
              <div className="p-6 flex flex-col justify-center items-center text-center text-gray-500 h-full">
                ...
                <p className="text-sm">
                  Recent client messages and calls will appear here.
                </p>
              </div>
            )}
          </div>
